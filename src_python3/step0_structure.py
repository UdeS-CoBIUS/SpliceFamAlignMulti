...
def launch_splign(cdsseqfilename, geneseqfilename, cdsid, geneid):
    ...
    splignoutputname = os.getcwd()+'/src_python3/results/splign_results/CDS_' + cdsid+'VS_Gene_' + geneid+".splign"

    os.system(os.getcwd()+"/src_python3/splign -query "+ cdsseqfilename + " -subj "+geneseqfilename+" -disc -aln "+splignoutputname+ " -min_compartment_idty 0.00 -min_exon_idty 0.00 > null")
            ...
            if len(currentHit) != 0:
                if len(blocks) > 0:
                    for j in range(0, len(blocks)):
                        if blocks[j][0] <= currentHit[0] and currentHit[0] <= blocks[j][1] and \
                                blocks[j][0] <= currentHit[1] and currentHit[1] <= blocks[j][1] and \
                                blocks[j][2] <= currentHit[2] and currentHit[2] <= blocks[j][3] and \
                                blocks[j][2] <= currentHit[3] and currentHit[3] <= blocks[j][3]:
                            break
                        elif (blocks[j][0] >= currentHit[1] and blocks[j][2] > currentHit[3]) or \
                                (currentHit[0] >= blocks[j][1] and currentHit[2] > blocks[j][3]):
                            if j == len(blocks) - 1:
...
def localAlignDP(bloc, cdsSeq, geneSeq):
    ...
    if len(begin) > 0:
        for i in range(len(begin)):
            Foundblocs.append([
                dicAlignmentPosandCdsGeneNt[begin[i]][0] + bloc[0], 
                dicAlignmentPosandCdsGeneNt[end[i]][0] + bloc[0], 
                dicAlignmentPosandCdsGeneNt[begin[i]][1] + bloc[2], 
                dicAlignmentPosandCdsGeneNt[end[i]][1] + bloc[2]
            ])
