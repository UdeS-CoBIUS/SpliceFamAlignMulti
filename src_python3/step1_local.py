...
def localAlignment(cdsId, cdsLen, cdsExon, geneId, geneLen, geneSeq):
    ...
    cdsFile = os.getcwd() + '/src_python3/sequences/cds/CDS_' + cdsId + '.fasta'
    geneFile = os.getcwd() + '/src_python3/sequences/genes/Gene_' + geneId + '.fasta'
...
def parseTblastxTrimBlock(cdsId, geneId, cdsExon, blastOutput, geneSeq, evalue):
    ...
    """
    This function takes tblastx results, checks hits compatibility 
    ...
    blastOutput: filename
        filename containing tblastx results (hits information)
    geneSeq: string
        gene sequence
    evalue: float
        cut-off value for tblastx significant results
    ...
    for hit in listHits:
        ...
        while continue_ == True and i <= len(listExonOfCDS) - 1:
            ...
            segmentIntersectionLast, lengthIntersection = determineIntersection(
                beginExon, endExon, int(beginNewHitCDS), int(endNewHitCDS), 
                int(beginNewHitgene), int(endNewHitgene), cdsId, geneId,
        ...
        if len(listHitsExon) == 1:
            ...
            for hitExon in listHitsExonInOrder:
                ...
                if len(dictExonBlocksToaffect[(beginExon, endExon)]) == 0:
                    ...
                    dictExonBlocksToaffect[(beginExon, endExon)] = affectHit(
                        dictExonBlocksToaffect[(beginExon, endExon)], NewHit, cdsId, geneId, evalueOfNewHit
                    )

    for i in range(0, len(list(dictExonBlocksToaffect.keys()))):
        for j in range(i + 1, len(list(dictExonBlocksToaffect.keys()))):
            for hitexon in list(dictExonBlocksToaffect.values())[i]:
                exon1 = list(dictExonBlocksToaffect.keys())[i]
                for hitotherexon in list(dictExonBlocksToaffect.values())[j]:
                    exon2 = list(dictExonBlocksToaffect.keys())[j]
    ...
    for cle, val in list(incompatibledictExonBlocksToaffect.items()):
        if len(val) == 0:
    ...
    for blockItem in list(compatibledictExonBlocksToaffect.values()):
        if len(blockItem) == 0:
            pass
        if len(blockItem) == 1:
...
def affectHit(dictExonBlocksNew, segmentIntersection, cdsId, geneId, evalueOfNewHit):
	...
	while HitAndEvalue < len(dictExonBlocksNew):
		...
		minDistanceCDS = int(abs(minBeginCds - maxBeginCds))
		minDistanceGene = int(abs(minBeginGene - maxBeginGene))
		compatible = 2
		case = ''
