...
def launch_tblastx(cdsfile, genefile,evalue, block1,block2,cdsid, geneid):
    ...
    blastoutput = str(os.getcwd()+'/src_python3/results/blast_results/CDS_'+ cdsid+'_'+str(block1_qe)+'-'+str(block2_qs)+'_VS_Gene_'+ geneid+'_'+str(block1_se)+'-'+str(block2_ss)+'.tblastx')
...
def readBlastOut(blastOutput, blockQueryEnd, blockSubjectEnd,evalue):
    ...
    with open(blastOutput, 'rb') as csvfile:
        spamreader = csv.reader(codecs.iterdecode(csvfile, 'utf-8'),delimiter = ' ', quotechar = ' ')
