...
def extendExtremities(cdsId, geneId, blockListextend, cdsSeq, geneSeq):
    ...
    for bloc in blockListextend:
        ...
        if(position == "middle"):
            ...
            for i in range(min(max_indel,int((geneEnd-geneBegin-cdsLength)/3))):
            ...
            for i in range(min(max_indel,int(cdsLength/3))):
...
def findSimilarExonsPD(resultsBloc, blockListDP, geneExon, cdsExon, intronList, cdsId, geneId, lenGene, lenCDS, cdsSeq, geneSeq):
    ...
    for bloci in extendedBlocks:
        ...
        # Identify extremity of two successive blocks
        [QueryStart, QueryEnd, SubjectStart, SubjectEnd] = bloci
        
        if (SubjectEnd - SubjectStart) > 0:
            (exonInGene, intronInGene, exonBlocksWithinCds, posExonStartGene, 
             posExonEndGene, posExonStartCds, posExonEndCds, posStartIntron, posEndIntron) = cdsGenerestrictionExonIntron(
                geneId, cdsId, geneExon, cdsExon, intronList, QueryStart, QueryEnd, SubjectStart, SubjectEnd
            )
