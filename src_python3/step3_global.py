...
def extracIntervalBlock(sourceDictionary, sourceId, blockInInterval, posNucleotideStartInInterval,\
    ...
    sourceDictionary: dictionary
        Dictionary that contains a list of cds exons or gene exons or gene introns.
        The keys are source IDs, and the values are the block intervals of exon/intron source.
    sourceId: string
        It can be a CDS ID or gene ID.

    blockInInterval: dictionary
        Dictionary that contains a list of CDS exons or gene exons or gene introns belonging to the inter block.
    posNucleotideStartInInterval: dictionary
        Dictionary that contains a list of start positions of CDS exons or gene exons or gene introns belonging to the inter block.
    posNucleotideEndInInterval: dictionary
        Dictionary that contains a list of end positions of CDS exons or gene exons or gene introns belonging to the inter block.
    beginInterval: int
        Start of the inter block.
    endInterval: int
    ...
    -------
    blockInInterval: dictionary
        Dictionary of lists of exons/introns.
    posNucleotideStartInInterval: dictionary
        Dictionary of start positions of nucleotides.
    posNucleotideEndInInterval: dictionary
        Dictionary of end positions of nucleotides.
    """
    ...
    if sourceId in sourceDictionary:
        values = sourceDictionary[sourceId]
        for i in range(len(values)):
            minInter = values[i][0]
            maxInter = values[i][1]

            if beginInterval <= minInter < endInterval:
                if beginInterval < maxInter <= endInterval:
...
def concatenate_exons(blocklist, cdsseq, geneseq):
    """
    This function concatenates all pairs of successive blocks whose cds_separation is less than
    MAX_CDS_SEPARATION_FOR_CONCAT, and gene_separation does not exceed MAX_CDS_SEPARATION_FOR_CONCAT
    +/- MAX_SEPARATION_DIFF_FOR_CONCAT.
    ...
    for i in range(len(blocklist) - 1):
        ...
        # Check if separation is within limits for concatenation
        if cdsseparation <= MAX_CDS_SEPARATION_FOR_CONCAT:
            ...
            if diff <= MAX_SEPARATION_DIFF_FOR_CONCAT and diff % 3 == 0:
                ...
                alignment = pairwise2.align.globalms(
                    geneseq[block1_se - 5:block2_ss + 5], 
                    cdsseq[block1_qe - 5:block2_qs + 5],
                    2.0, 0.0, -5.0, -1.0
                )
