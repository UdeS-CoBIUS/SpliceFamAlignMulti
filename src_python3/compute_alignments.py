...
def spliceAlignment(sourceData, targetData,  step, compareExon, choice, pairwiseMethod, outputPrefix):
    ...
    for cds in sourceData:
        ...
        cdsSeqFile = os.getcwd() + '/src_python3/sequences/cds/CDS_' + cdsId + '.fasta'
    ...
    for gene in targetData:
        geneId, geneSeq = gene
        geneSeqFile = os.getcwd() + '/src_python3/sequences/genes/Gene_' + geneId + '.fasta'
    ...
    for cds in sourceData:
        ...
        cdsExon[cdsId] = []
        cdsSeqFile = os.getcwd() + '/src_python3/sequences/cds/CDS_' + cdsId + '.fasta'
        geneSeqFile = os.getcwd() + '/src_python3/sequences/genes/Gene_' + cdsGeneId  + '.fasta'
        # if no splicing structure information was given in input
        if (len(blockList) == 0):
            if  choice == 'splign':
                ...
                print('choose method to extract exon structure by splign or blast')
                exit(-1)
    ...
    for cds in sourceData:
        cdsId, cdsSeq, null, null = cds
        cdssSeqFile = os.getcwd() + '/src_python3/sequences/cds/CDS_' + cdsId + '.fasta'
